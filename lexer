...
function CONSTRUCT_Token() -- this just creates a new token object because lua has no oop
  return { -- you can also put args into the consturct function instead of doing defaults
    ctype = "token",
    ttype = "", -- i like to use enums but lua has no custom enum feature 😡
...
local function lex(str)
  ...
  while i < str:len() do -- you can NOT do :split(""), the i in here is important as some tokens are multiple characters
    ...
    elseif string.find("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ_",esc(char)) then
        local val = char
        while i < str:len() and string.find("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ_1234567890",esc(str:sub(i+1,i+1))) do
            i=i+1;
            local c2 = str:sub(i,i)
            val = val..c2
        end
        local tok = CONSTRUCT_Token()
        tok.ttype = table.find(keywords,val) and "keyword" or "identifier"
        tok.value = val
        if val == "." then tok.ttype = "period" end
        table.insert(tokens,tok)
...
setFontSize(34)
local tok = lex('local a = 36^2')
for i,v in pairs(tok) do
    print(string.format("type: %s value: %s",v.ttype,v.value))
end
